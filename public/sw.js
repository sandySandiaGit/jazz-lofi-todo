const VERSION = "59"; // Change it to force a hard cache update!
...
self.addEventListener("install", (event) => {
  ...
  event.waitUntil(
    (async () => {
      ...
      // 2. VITE TRICK: Parse index.html to find and cache hashed assets (JS/CSS):
      try {
        ...
        while ((match = regex.exec(html)) !== null) {
          ...
          await cache.add(match[1]); // Add the JS/CSS files generated by Vite
        }
      } catch (err) {
...
self.addEventListener("activate", (event) => {
  ...
  event.waitUntil(
    (async () => {
      ...
      // Fail-safe fallback to preserve old caches if the current installation is completely empty:
      if (currentKeys.length === 0) {
        console.warn("[SW] New cache is empty, preserving legacy caches.");
        return; 
      }
